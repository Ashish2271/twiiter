...
const extraResolvers = {
  User: {
    likes: (parent: User) => UserService.getLikes(parent.id),
    liked: async (parent: User, { tweetId }: { tweetId: string }, ctx: GraphqlContext) => {
      if (!ctx.user || !ctx.user.id) {
        throw new Error("unauthenticated");
      }

      // Fetch the liked tweets for the current user
      const likedTweets = await UserService.getLikes(ctx.user.id);
